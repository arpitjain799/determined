...
def test_rbac_list_roles() -> None:
    with logged_in_user(ADMIN_CREDENTIALS):
        ...
        non_global_roles = det_cmd_json(["rbac", "list-roles", "--exclude-global-roles", "--json"])[
            "roles"
        ]
        non_global_role_ids = {r["roleId"] for r in non_global_roles}
        for role in all_roles:
            is_excluded = role["roleId"] not in non_global_role_ids
